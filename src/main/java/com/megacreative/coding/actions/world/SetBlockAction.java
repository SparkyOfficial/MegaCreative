...
public class SetBlockAction implements BlockAction {
    ...
    private SetBlockParams getBlockParamsFromContainer(CodeBlock block, ExecutionContext context) {
        ...
        try {
            BlockConfigService blockConfigService = context.getPlugin()
                    .getServiceRegistry()
            ...
            context.getPlugin().getLogger().warning(
                    "Error getting block parameters from container in SetBlockAction: " + e.getMessage()
            );
        }
    ...
    private int getRelativeXFromItem(ItemStack item, int defaultValue) {
        return parseRelativeCoordinate(item, defaultValue);
    }
    ...
    /** Helper method to parse coordinates from item display name or fallback to amount */
    private int parseRelativeCoordinate(ItemStack item, int defaultValue) {
        try {
