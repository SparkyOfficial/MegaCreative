 ...
 * This action divides an existing variable by a value from container configuration.
 */
    ...
    public ExecutionResult execute(CodeBlock block, ExecutionContext context) {
        try {
            ...
            if (variableManager != null) {
                ...
                if (variableValue != null) {
                    try {
                        // Try to divide as numbers
                        double varNum = Double.parseDouble(variableValue.asString());
                        double divNum = Double.parseDouble(resolvedValue.asString());
                        
                        if (divNum == 0) {
                            return ExecutionResult.error("Cannot divide by zero");
                        }
                        
                        double result = varNum / divNum;
                        
                        // Set the new value
                        variableManager.setVariable(resolvedVarName, DataValue.of(result), variableScope, variableContext);
                        return ExecutionResult.success("Divided variable '" + resolvedVarName + "' by " + divNum + ", result: " + result);
                    } catch (NumberFormatException e) {
                        return ExecutionResult.error("Cannot divide non-numeric variable '" + resolvedVarName + "'");
                    }
                } else {
