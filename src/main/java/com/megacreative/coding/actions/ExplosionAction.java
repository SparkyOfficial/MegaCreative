...
public class ExplosionAction implements BlockAction {
            ...
            if (slotResolver != null) {
                ...
                if (breakBlocksSlot != null) {
                    ...
                    if (breakBlocksItem != null && breakBlocksItem.hasItemMeta()) {
                        // Extract breakBlocks from item
                        params.breakBlocks = getBooleanFromItem(breakBlocksItem, true);
                    }
                }
            }
        } catch (Exception e) {
            context.getPlugin().getLogger().warning("Error getting explosion parameters from container in ExplosionAction: " + e.getMessage());
        }
        ...
        try {
            ...
            if (meta != null) {
                ...
                if (displayName != null && !displayName.isEmpty()) {
                    // Try to parse float from display name
    ...
    private boolean getBooleanFromItem(ItemStack item, boolean defaultValue) {
        try {
            ...
            // Fallback to item amount (odd = true, even = false)
